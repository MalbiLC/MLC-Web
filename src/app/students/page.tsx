      ...
      <div className="page-content space-y-4">
        <div className="flex gap-1 border-b border-gray-100">
          {(['current', 'potential'] as const).map(t => (
            <button key={t} onClick={() => { setTab(t); setSearch('') }} className={cn(
        ...
        <div className="relative max-w-xs">
          ...
          <input type="text" className="input pl-9" placeholder="Search students…" value={search} onChange={e => setSearch(e.target.value)} />
        </div>
        {loading ? <div className="text-sm text-gray-400 py-8">Loading…</div> : (
          <div className="table-container">
            <table>
              ...
              <tbody>
                {filtered.length === 0 ? (
                  <tr><td colSpan={7} className="text-center text-gray-400 py-10">{search ? 'No results.' : `No ${tab} students yet.`}</td></tr>
                ) : filtered.map((s, i) => {
                  const badge = studentStatusConfig[s.status]
                  const isLow = s.sessions_remaining <= 3 && s.sessions_remaining > 0
                  return (
                    <tr key={s.id}>
                      <td>
                        <div className="flex items-center gap-2.5">
                          <div className={cn('w-7 h-7 rounded-full flex items-center justify-center text-xs font-medium shrink-0', colors[i % colors.length])}>{getInitials(s.full_name)}</div>
                          <div>
                      ...
                      <td>
                        <div className="flex flex-wrap gap-1">
                          {s.subjects?.length ? s.subjects.map(sub => (
                            <span key={sub.id} className="badge bg-gray-100 text-gray-600">{sub.name}</span>
                      ...
                      {tab === 'current' && (
                        <td><span className={cn('font-medium', s.sessions_remaining === 0 ? 'text-red-600' : isLow ? 'text-amber-600' : 'text-gray-700')}>{s.sessions_remaining}</span>{isLow && <span className="ml-1 text-xs text-amber-500">Low</span>}</td>
                      )}
                      {tab === 'current' && badge && (
                        <td><span className={cn('badge', badge.color)}>{badge.label}</span></td>
                      )}
                      {tab === 'potential' && (
