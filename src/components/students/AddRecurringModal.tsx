...
import { createRecurringStudent } from '@/lib/studentQueries'
...
export default function AddRecurringModal({ onClose, onSuccess }: Props) {
  const [form, setForm] = useState({
    full_name: '', date_of_birth: '', parent_name: '',
    parent_contact: '', package: 'private', additional_notes: '',
  })
  ...
  const addSubjectRow = () => setSubjects(r => [...r, { subject_id: '', teacher_id: '', sessions: 0 }])
  const removeSubjectRow = (i: number) => setSubjects(r => r.filter((_, idx) => idx !== i))
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl w-full max-w-lg shadow-xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 sticky top-0 bg-white">
          <h2 className="text-base font-semibold text-gray-900">Add recurring student</h2>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <button type="button" onClick={addSubjectRow} className="text-xs text-gray-500 hover:text-gray-900 flex items-center gap-1 transition-colors">
            ...
            <div className="space-y-2">
              {subjects.map((row, i) => (
                <div key={i} className="grid grid-cols-[1fr_1fr_80px_32px] gap-2 items-center">
                  ...
                  <input className="input text-center" type="number" min="0" placeholder="0" value={row.sessions || ''}
                    onChange={e => updateSubject(i, 'sessions', parseInt(e.target.value) || 0)} />
                  {subjects.length > 1 && (
                    <button type="button" onClick={() => removeSubjectRow(i)} className="text-gray-300 hover:text-red-500 transition-colors flex justify-center">
                      <Trash2 size={15} />
                    </button>
                  )}
                </div>
