...
export default function PotentialTable({ students, onRefresh }: Props) {
  ...
  const markTrialDone = async (id: string) => {
    setUpdating(id)
    try {
      await updateStudentStatus(id, 'potential_trial_done')
      onRefresh()
    } finally {
      setUpdating(null)
    }
  ...
  if (students.length === 0) {
    return (
      <div className="table-container">
        <div className="text-center text-gray-400 text-sm py-16">No potential students yet.</div>
      </div>
    )
  }
              ...
              <td>
                {s.status === 'potential_no_trial' && (
                  <button
                    ...
                    className={cn(
                      'text-xs px-3 py-1.5 rounded-lg border font-medium transition-colors whitespace-nowrap',
